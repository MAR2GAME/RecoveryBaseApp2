...
import com.pdffox.adv.use.parseJson
...
object AdvCheckManager {
	...
	private fun checkIpInfoLegacy(): String? {
		PreferenceUtil.init(AdvApplicaiton.instance)
	...
	private fun applyIpInfoResult(parsed: IpGeoDetail?, source: String): String {
		...
		if (BuildConfig.DEBUG && parsed != null) {
			val detailSummary =
				"IP detail -> ip=${parsed.ip}, longitude=${parsed.longitude}, latitude=${parsed.latitude}, asn=${parsed.asn}, isp=${parsed.isp}"
	...
	private fun loadCidrsFromRaw(resId: Int): List<CidrRange> {
		val context = AdvApplicaiton.instance
		return runCatching {
	...
	fun checkToken(token: String) {
		...
		val params = JSONObject().apply {
			...
			put("PackageName", AdvApplicaiton.instance.packageName)
			put("token", token)
		}
...
//					}
//					if (requestPackageName != AdvApplicaiton.instance.packageName){
