...
class RecoveryViewModel(
	...
	fun setCurrenntFold(scanType: String, foldBean: FoldBean) {
		...
		recoveryRepository.cacheCurrentFold(scanType, foldBean)
		getFoldSortBeans()
		viewModelScope.launch(Dispatchers.IO) {
...
//		getFoldSortBeans()
	...
	suspend fun resolveCurrentFold(scanType: String = "recovery_photos", foldName: String): FoldBean {
		_currentScanType.value = scanType
		val savedFold = recoveryRepository.getCurrentFold(scanType)
		val resolvedFold = if (savedFold.name == foldName && savedFold.files.isNotEmpty()) {
			savedFold
		} else {
			recoveryRepository.getRecoveryFold(scanType, foldName)
		}
		_currentFold.value = resolvedFold
		recoveryRepository.cacheCurrentFold(scanType, resolvedFold)
		getFoldSortBeans()
		return resolvedFold
	}
