...
import com.datatool.photorecovery.core.bean.AllCapacityInfo
...
class RecoveryRepository(
	...
	private val currentFoldCache = mutableMapOf<String, FoldBean>()
...
//	}
	...
	suspend fun saveCurrentFold(scanType: String = "recovery_photos", currentFold: FoldBean) {
		currentFoldCache[scanType] = currentFold
	...
	fun cacheCurrentFold(scanType: String = "recovery_photos", currentFold: FoldBean) {
		currentFoldCache[scanType] = currentFold
	}

	suspend fun getCurrentFold(scanType: String = "recovery_photos"): FoldBean {
		currentFoldCache[scanType]?.let { return it }
		val key = stringPreferencesKey("current_fold_$scanType")
		val currentFold = dataStore.data.map { prefs ->
	...
	suspend fun getRecoveryFold(scanType: String = "recovery_photos", foldName: String): FoldBean {
		return getRecoveryFiles(scanType).firstOrNull { it.name == foldName } ?: FoldBean(name = foldName)
	}
